...
def _sum_dir_size_recursively_if(dir: Directory) -> int:
    dirs_sum = 0
    for item in dir.childrens:
        if type(item) is Directory:
            if (dir_size := _get_size_of_dir(item)) < 100000:
                print(dir_size)
                dirs_sum += dir_size
            dirs_sum += _sum_dir_size_recursively_if(item)
...
#    input_file = [l for n, l in enumerate(input_file) if n < int(num_of_lines)]
    ...
    print(_sum_dir_size_recursively_if(directories_context[0]))
